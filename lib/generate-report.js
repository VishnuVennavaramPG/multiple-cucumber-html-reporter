...
const RESULTS_SUMMARY_TEMPLATE = 'components/results-summary.tmpl';
...
function generateReport(options) {
  ...
  suite.resultsSummary = processAllFeatures();

  function processAllFeatures() {
    ...
    results.title = "Results summary by Country and platform"
    /*console.log("-------------------------------------------------------------------------")
    ...
    function getObjectWithPercentagesBasedOnCountryForFeatures(filteredFeatures) {
      let suite = getSuiteDetailsBasedOnParsedFeatures(filteredFeatures);

      const percentageObject=  {
        "features-pass%": suite.featureCount.passedPercentage,
        "features-fail%": suite.featureCount.failedPercentage,
        "features-count" : suite.featureCount.total,
        "scenarios-pass%": suite.scenarios.passedPercentage,
        "scenarios-fail%": suite.scenarios.failedPercentage,
        "Pass": suite.scenarios.passed,
        "Fail": suite.scenarios.failed,
        "scenarios-count": suite.scenarios.total,
      }
  ...
  function _createFeaturesOverviewIndexPage(suite) {
    ...
    fs.writeFileSync(
      ...
      _.template(_readTemplateFile(FEATURES_OVERVIEW_INDEX_TEMPLATE))({
        ...
        resultsSummaryOverview: _.template(_readTemplateFile(RESULTS_SUMMARY_TEMPLATE))(
            {
              suite: suite,
              _: _,
            }
