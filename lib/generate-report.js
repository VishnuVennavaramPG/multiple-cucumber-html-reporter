...
function generateReport(options) {
  ...
  let SG_Android_suite = getObjectWithPercentagesBasedOnCountryForFeatures(allFeatures, "SG", "android");
  let SG_ios_suite = getObjectWithPercentagesBasedOnCountryForFeatures(allFeatures, "SG", "ios");
  ...
  function getObjectWithPercentagesBasedOnCountryForFeatures(features, country, platform) {
    let featuresGrouped = filterFeaturesByCountryAndPlatform(features, country, platform)

    if (featuresGrouped.length === 0) {
      console.log(country + "_ " + platform + " : " + "No tests found")
      return {
      };
    }
    let suite = {
...
=====================================================================================\n`
  ...
  function filterFeaturesByCountryAndPlatform(features, country, platform) {
    return features.filter(feature => {
      let metadata = feature.metadata;
      let metadataCountry = metadata.find(item => item.name === 'Country')?.value;
      let metadataPlatform = metadata.find(item => item.name === 'Platform')?.value;
      return metadataCountry === country && metadataPlatform === platform;
    });
  }
