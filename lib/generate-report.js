...
const COUNTRY_PLATFORM_OVERVIEW_CHART_TEMPLATE =
  'components/country-platform-overview.chart.tmpl';
...
function generateReport(options) {
  ...
  let suite = getSuiteDetailsBasedOnParsedFeatures(allFeatures);
  function getSuiteDetailsBasedOnParsedFeatures(features) {
    let suite = {
      app: 0,
...
suite.subFeatures = processAllFeatures(allFeatures);

  function processAllFeatures(features, country, platform) {
    // First, determine all unique combinations of country and platform
    let combinations = {};
    for (let feature of allFeatures) {
      let country = feature.metadata.find(item => item.name === 'Country')?.value;
      let platform = feature.metadata.find(item => item.name === 'Platform')?.value;
      let key = `${country}-${platform}`;
      ...
      combinations[key].features.push(feature);
      
      //GM0917: This can just be called without condition since we always create a key when its not found
      // Only add the feature to the combination if it matches the country and platform
      // if (country === combinations[key].country && platform === combinations[key].platform) {
      //   combinations[key].features.push(feature);
      // }
    }

    // Then, calculate the percentages for each combination
    let results = {};
    for (let key in combinations) {
      let {country, platform, features} = combinations[key];
      results[key] = getObjectWithPercentagesBasedOnCountryForFeatures(features, country, platform);
    }
    console.log("-------------------------------------------------------------------------")
  ...
  // Process the features and generate percentages required for short logs
  function getObjectWithPercentagesBasedOnCountryForFeatures(filteredFeatures, country, platform) {
    let suite = getSuiteDetailsBasedOnParsedFeatures(filteredFeatures);
    ...
    const percentageObject=  {
      ...
      features: {
        ...
        total : suite.featureCount.total,
        passedPercentage: suite.featureCount.passedPercentage,
        failedPercentage: suite.featureCount.failedPercentage
      },
      scenarios: {
        passed: suite.scenarios.passed,
        failed: suite.scenarios.failed,
        total: suite.scenarios.total,
        passedPercentage: suite.scenarios.passedPercentage,
        failedPercentage: suite.scenarios.failedPercentage
      }
    }

    // console.log(country + "_" + platform + " : " + JSON.stringify(percentageObject));
  ...
  if (saveCollectedJSON) {
    ...
    jsonFile.writeFileSync(path.resolve(reportPath, 'group-summary.json'),
      suite.subFeatures,
      { spaces: 2 }
    );
  }
  ...
  function _createFeaturesOverviewIndexPage(suite) {
    ...
    fs.writeFileSync(
      ...
      _.template(_readTemplateFile(FEATURES_OVERVIEW_INDEX_TEMPLATE))({
        ...
        sgAndroidScenarioOverviewChart: _.template(
          _readTemplateFile(COUNTRY_PLATFORM_OVERVIEW_CHART_TEMPLATE)
        )({
          overviewPage: true,
          scenarios: suite.subFeatures["SG-android"]["scenarios"],
          suite: suite.subFeatures["SG-android"],
          _: _,
        }),
